...
- 🖼️ **GIF Preview** - See trick animations directly in the editor
...
- Pillow (optional but recommended for GIF previews)
...
2. Install optional dependencies (recommended):
   ```bash
   pip install -r requirements-trick-editor.txt
   ```
   
   Or just install Pillow:
   ```bash
   pip install Pillow
   ```
...
- Or double-click `run_trick_editor.bat`
...
3. **View the GIF preview** if the trick has a GIF URL
4. **Click "💾 Save Changes"** to save the trick (this updates the in-memory data)
5. **Click "💾 Save Changes"** in the sidebar to write to the JSON file
...
The editor automatically displays a preview of the trick's GIF if:
- The trick has a `gifUrl` field set
- The GIF file exists locally or is accessible via URL
- Pillow is installed (for best results)

The preview is displayed in the "GIF PREVIEW" section below the basic info fields.
...
### GIF Previews Not Showing

If GIF previews aren't displaying:
...
2. Check that the GIF URL is correct:
   - For local files: relative path like `/JugglingGifs/3balltricks/cascade.gif`
   - For web URLs: full URL starting with `http://` or `https://`

3. Ensure the GIF file exists in the static folder
...
- ✅ Native file dialogs
- ✅ GIF previews built-in
- ✅ Toast notifications instead of blocking popups

**Trade-offs:**
- ⚠️ Requires Python installation
- ⚠️ Native desktop UI instead of web-based
- ⚠️ Pillow recommended for best GIF support
